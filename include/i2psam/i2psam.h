...
static constexpr std::size_t SAM_BUFSIZE = 8192;
static constexpr std::string_view SAM_DEFAULT_ADDRESS = "127.0.0.1";
static constexpr std::size_t SAM_DEFAULT_PORT_TCP = 7656;
static constexpr std::size_t SAM_DEFAULT_PORT_UDP = 7655;
static constexpr std::size_t SAM_DEFAULT_CLIENT_TCP = 7666;
static constexpr std::size_t SAM_DEFAULT_CLIENT_UDP = 7667;
static constexpr std::string_view SAM_GENERATE_MY_DESTINATION = "TRANSIENT";
static constexpr std::string_view SAM_MY_NAME = "ME";
static constexpr auto SAM_DEFAULT_I2P_OPTIONS =
    concat("i2cp.leaseSetEnctype", "=", "0,4"); // i2cp.leaseSetEncType=0,4
static constexpr std::string_view
    SAM_DEFAULT_I2P_OPTIONS_VIEW(SAM_DEFAULT_I2P_OPTIONS.data(),
                                 SAM_DEFAULT_I2P_OPTIONS.size() - 1);
static constexpr std::string_view SAM_SIGNATURE_TYPE = "EdDSA_SHA512_Ed25519";

static constexpr std::string_view SAM_NAME_INBOUND_QUANTITY =
    "inbound.quantity";
static constexpr std::size_t SAM_DEFAULT_INBOUND_QUANTITY =
    3; // Three tunnels is default now
static constexpr std::string_view SAM_NAME_INBOUND_LENGTH = "inbound.length";
static constexpr std::size_t SAM_DEFAULT_INBOUND_LENGTH =
    3; // Three jumps is default now
static constexpr std::string_view SAM_NAME_INBOUND_LENGTHVARIANCE =
    "inbound.lengthVariance";
static constexpr std::size_t SAM_DEFAULT_INBOUND_LENGTHVARIANCE = 0;
static constexpr std::string_view SAM_NAME_INBOUND_BACKUPQUANTITY =
    "inbound.backupQuantity";
static constexpr std::size_t SAM_DEFAULT_INBOUND_BACKUPQUANTITY =
    1; // One backup tunnel
static constexpr std::string_view SAM_NAME_INBOUND_ALLOWZEROHOP =
    "inbound.allowZeroHop";
static constexpr bool SAM_DEFAULT_INBOUND_ALLOWZEROHOP = true;
static constexpr std::string_view SAM_NAME_INBOUND_IPRESTRICTION =
    "inbound.IPRestriction";
static constexpr std::size_t SAM_DEFAULT_INBOUND_IPRESTRICTION = 2;
static constexpr std::string_view SAM_NAME_OUTBOUND_QUANTITY =
    "outbound.quantity";
static constexpr std::size_t SAM_DEFAULT_OUTBOUND_QUANTITY = 3;
static constexpr std::string_view SAM_NAME_OUTBOUND_LENGTH = "outbound.length";
static constexpr std::size_t SAM_DEFAULT_OUTBOUND_LENGTH = 3;
static constexpr std::string_view SAM_NAME_OUTBOUND_LENGTHVARIANCE =
    "outbound.lengthVariance";
static constexpr std::size_t SAM_DEFAULT_OUTBOUND_LENGTHVARIANCE = 0;
static constexpr std::string_view SAM_NAME_OUTBOUND_BACKUPQUANTITY =
    "outbound.backupQuantity";
static constexpr std::size_t SAM_DEFAULT_OUTBOUND_BACKUPQUANTITY = 1;
static constexpr std::string_view SAM_NAME_OUTBOUND_ALLOWZEROHOP =
    "outbound.allowZeroHop";
static constexpr bool SAM_DEFAULT_OUTBOUND_ALLOWZEROHOP = true;
static constexpr std::string_view SAM_NAME_OUTBOUND_IPRESTRICTION =
    "outbound.IPRestriction";
static constexpr std::size_t SAM_DEFAULT_OUTBOUND_IPRESTRICTION = 2;
static constexpr std::string_view SAM_NAME_OUTBOUND_PRIORITY =
    "outbound.priority";
static constexpr std::string_view SAM_DEFAULT_OUTBOUND_PRIORITY;
static constexpr std::string_view SAM_NAME_I2CP_LEASESET_ENC_TYPE =
    "i2cp.leaseSetEncType";
static constexpr std::string_view SAM_DEFAULT_I2CP_LEASESET_ENC_TYPE = "0,4";
...
I2PSAM_NO_EXPORT static inline void print_error(const std::string &err) {
...
class I2PSAM_EXPORT Message {
public:
  enum SessionStyle : uint8_t { sssStream, sssDatagram, sssRaw };
  ...
  template <class T> struct Answer {
    ...
    // static constexpr T default_value_ = Default;

    const Message::eStatus status_;
    T value_;
  ...
  // Stream session
  static std::string sessionCreate(
      SessionStyle style, const std::string &sessionID,
      const std::string &nickname,
      const std::string &destination = SAM_GENERATE_MY_DESTINATION.data(),
      const std::string &options = "",
      const std::string &signatureType = SAM_SIGNATURE_TYPE.data());
  ...
  static std::string 
  datagramSend(const std::string &nickname,
                                  const std::string &destination/*,
...
private:
  ...
  static std::string createSAMRequest(const char *msg) {
    return {msg};
  }

  template <typename... t_args>
  static std::string createSAMRequest(const char *format, t_args &&...args) {
...
class I2PSAM_EXPORT I2pSocket {
public:
  I2pSocket(const std::string &SAMHost, uint16_t SAMPort);
  I2pSocket(const sockaddr_in &addr); // explicit because we don't want to
                                      // create any socket implicity
  explicit I2pSocket(
...
struct I2PSAM_EXPORT FullDestination {
  std::string pub_;
  std::string priv_;
  bool isGenerated_;
  ...
  FullDestination(const std::string &pub, const std::string &priv,
                  bool isGenerated)
      : pub_(pub), priv_(priv), isGenerated_(isGenerated) {}
};

template <class T> struct I2PSAM_EXPORT RequestResult {
  // static constexpr T value_type;

  bool isOk_;
  T value_;

  RequestResult() : isOk_(false) {}

  explicit RequestResult(const T &value) : isOk_(true), value_(value) {}
};

template <class T> struct I2PSAM_EXPORT RequestResult<std::unique_ptr<T>> {
  /**
  ...
  struct RequestResultRef {
    ...
    RequestResultRef(bool isOk, T *value) : isOk_(isOk), value_(value) {}
  };
...
template <> struct I2PSAM_EXPORT RequestResult<void> {
  bool isOk_;

  RequestResult() : isOk_(false) {}

  explicit RequestResult(bool isOk) : isOk_(isOk) {}
};

class I2PSAM_EXPORT SAMSession {
public:
  SAMSession(
      const std::string &nickname,
      const std::string &SAMHost = SAM_DEFAULT_ADDRESS.data(),
      uint16_t SAMPort = SAM_DEFAULT_PORT_TCP,
      const std::string &i2pOptions = SAM_DEFAULT_I2P_OPTIONS_VIEW.data(),
      const std::string &signatureType = SAM_SIGNATURE_TYPE.data());
  ...
  virtual FullDestination createSession(const std::string &destination,
                                        const std::string &sigType,
                                        const std::string &i2pOptions) = 0;
...
class I2PSAM_EXPORT StreamSession final : public SAMSession {
public:
  StreamSession(
      const std::string &nickname,
      const std::string &SAMHost = SAM_DEFAULT_ADDRESS.data(),
...
private:
  ...
  struct ForwardedStream {
    std::unique_ptr<I2pSocket> *socket;
    std::string host;
    uint16_t port;
    bool silent;

    // ForwardedStream(std::unique_ptr<I2pSocket> socket, const std::string &host, uint16_t port,
    //                 bool silent)
    //     : socket(socket), host(host), port(port), silent(silent) {}

    ForwardedStream(std::unique_ptr<I2pSocket> *socket, const std::string &host, uint16_t port,
                    bool silent)
        : socket(std::move(socket)), host(std::move(host)),
          port(port), silent(silent) {}
  };
...
class I2PSAM_EXPORT DatagramSession final : public SAMSession {
public:
  DatagramSession(
      const std::string &nickname,
      const std::string &SAMHost = SAM_DEFAULT_ADDRESS.data(),
      ...
      const std::string &ClientAddress = SAM_DEFAULT_ADDRESS.data(),
...
class I2PSAM_EXPORT RawSession final : public SAMSession {
public:
  RawSession(
      const std::string &nickname,
      const std::string &SAMHost = SAM_DEFAULT_ADDRESS.data(),
